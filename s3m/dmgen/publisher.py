    ...
    if enumList:
        if len(tips) != len(enumList):
            if len(tips) == 0:
                ...
                reset_publication(self)
                msg = ("Cannot publish: "+self.__str__()+" The number of Enumerations and Definitions must be same. Check for empty lines.", messages.ERROR)
                return msg
...
def publish_DvFile(self):
    ...
    if self.content_mode == 'select':
        ...
        dt_str += padding.rjust(indent+8) + ("<xs:element maxOccurs='1' minOccurs='1' name='text-content' type='xs:string'/>\n")
...
def publish_DvInterval(self):
    ...
    if self.units_name and self.units_uri: # both must be present or they are both ignored.
...
def publish_DvOrdinal(self):
    ...
    for n in range(len(tips)):
        ...
        dt_str += padding.rjust(indent+16) + ("<xs:annotation>\n")
        dt_str += padding.rjust(indent+18) + ("<xs:appinfo>\n")
        dt_str += padding.rjust(indent+18) + ("<rdf:Description rdf:about='s3m:mc-"+self.ct_id+"/symbol/"+quote(s[n].strip())+"'>\n")
...
def publish_DvQuantity(self):
    ...
    if not mag_constrained:
        ...
        if (self.total_digits != None and self.total_digits >= 0):
            dt_str += padding.rjust(indent+12) + ("<xs:totalDigits value='"+str(self.total_digits).strip()+"'/>\n")
        if (self.fraction_digits != None and self.fraction_digits >= 0):
...
def publish_DvRatio(self):
    ...
    if self.num_units is not None and self.den_units is not None:
        if self.num_units.ct_id == self.den_units.ct_id:
            reset_publication(self)
            msg = (self.__str__()+": Numerator and denominator units must use different PCMs.", messages.ERROR)
            return msg

    if self.num_units is not None and self.ratio_units is not None:
        if self.num_units.ct_id == self.ratio_units.ct_id:
            reset_publication(self)
            msg = (self.__str__()+": Numerator and ratio units must use different PCMs.", messages.ERROR)
            return msg

    if self.den_units is not None and self.ratio_units is not None:
        if self.den_units.ct_id == self.ratio_units.ct_id:
            reset_publication(self)
            msg = (self.__str__()+": Denominator and ratio units must use different PCMs.", messages.ERROR)
            return msg
...
def publish_Party(self):
    ...
    if len(self.pred_obj.all()) != 0:
        for po in self.pred_obj.all():
            party_str += padding.rjust(indent+2) + ("<rdf:Description rdf:about='s3m:mc-"+self.ct_id+"'>\n")
...
def publish_Audit(self):
    ...
    if len(self.pred_obj.all()) != 0:
        for po in self.pred_obj.all():
            aud_str += padding.rjust(indent+2) + ("<rdf:Description rdf:about='s3m:mc-"+self.ct_id+"'>\n")
...
def publish_Attestation(self):
    ...
    if len(self.pred_obj.all()) != 0:
        for po in self.pred_obj.all():
            att_str += padding.rjust(indent+2) + ("<rdf:Description rdf:about='s3m:mc-"+self.ct_id+"'>\n")
...
def publish_Participation(self):
    ...
    if len(self.pred_obj.all()) != 0:
        for po in self.pred_obj.all():
            ptn_str += padding.rjust(indent+2) + ("<rdf:Description rdf:about='s3m:mc-"+self.ct_id+"'>\n")
    ...
    ptn_str += padding.rjust(indent+8) + ("<xs:element maxOccurs='1' minOccurs='0' name='end' type='xs:dateTimeStamp'/>\n")
...
def publish_Cluster(self):
    ...
    if len(self.pred_obj.all()) != 0:
        for po in self.pred_obj.all():
            cl_str += padding.rjust(indent+2) + ("<rdf:Description rdf:about='s3m:mc-"+self.ct_id+"'>\n")
...
def publish_Entry(self):
    ...
    #Create the datatype
    entry_str += padding.rjust(indent) + ("<xs:complexType name='mc-"+self.ct_id+"'> \n")
    ...
    if len(self.pred_obj.all()) != 0:
        for po in self.pred_obj.all():
            entry_str += padding.rjust(indent+2) + ("<rdf:Description rdf:about='s3m:mc-"+self.ct_id+"'>\n")
            entry_str += padding.rjust(indent+2) + ("<rdfs:subClassOf rdf:resource='s3m:EntryType'/>\n")
            entry_str += padding.rjust(indent+2) + ("<rdfs:label>"+escape(self.label.strip())+"</rdfs:label>\n")
            entry_str += padding.rjust(indent+2) + ("<"+po.predicate.ns_abbrev.__str__()+":"+po.predicate.class_name.strip()+" rdf:resource='"+ quote(po.object_uri) +"'/>\n")
    ...
    entry_str += padding.rjust(indent+4) + ("<xs:restriction base='s3m:EntryType'>\n")
    entry_str += padding.rjust(indent+6) + ("<xs:sequence>\n")
    entry_str += padding.rjust(indent+8) + ("<xs:element maxOccurs='1' minOccurs='1' name='label' type='xs:string' fixed="+'"'+escape(self.label.strip())+'"'+"/>\n")
    entry_str += padding.rjust(indent+8) + ("<xs:element maxOccurs='1' minOccurs='1' name='entry-language' type='xs:language' fixed='"+self.language+"'/>\n")
    entry_str += padding.rjust(indent+8) + ("<xs:element maxOccurs='1' minOccurs='1' name='entry-encoding' type='xs:string' fixed='"+self.encoding+"'/>\n")
    entry_str += padding.rjust(indent+8) + ("<xs:element maxOccurs='1' minOccurs='0' name='current-state' type='xs:string' default='"+escape(self.state)+"'/>\n")
    if not self.data:
        reset_publication(self)
        msg = (self.__str__()+": You cannot publish an Entry without a data element composed as a Cluster.", messages.ERROR)
        return msg
    else:
    ...
    if not self.provider:
        reset_publication(self)
        msg = ("Entry "+self.__str__().strip()+" must have a provider defined.", messages.ERROR)
        return msg
    else:
    ...
    if not self.participations.all():
        entry_str += padding.rjust(indent+8) + ("<!-- No Participations -->\n")
    else:
