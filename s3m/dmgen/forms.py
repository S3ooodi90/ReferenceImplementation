    ...
    def clean(self):
        ...
        if  cleaned_data['enums'] and not cleaned_data['definitions']:
            ...
            if len(enums.splitlines()) != len(definitions.splitlines()):
                if len(definitions.splitlines()) == 1: # allow one def to be replicated for all enums
...
class DvTemporalAdminForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (cleaned_data['allow_duration'] and cleaned_data['allow_ymduration']) or (cleaned_data['allow_duration'] and cleaned_data['allow_dtduration']) or (cleaned_data['allow_ymduration'] and cleaned_data['allow_dtduration']):
            raise forms.ValidationError("Only one of the duration types are allowed to be selected.")
...
class DMAdminForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        if instance.pk != None:
            self.fields['pred_obj'].queryset = PredObj.objects.filter(project=instance.project)
            self.fields['admin_definition'].queryset = AdminEntry.objects.filter(project=instance.project)
            self.fields['care_definition'].queryset = CareEntry.objects.filter(project=instance.project)
            self.fields['demog_definition'].queryset = DemographicEntry.objects.filter(project=instance.project)
        else:
            self.fields['pred_obj'].queryset = PredObj.objects.filter(project=self.default_prj)
            self.fields['admin_definition'].queryset = AdminEntry.objects.filter(project=self.default_prj)
            self.fields['care_definition'].queryset = CareEntry.objects.filter(project=self.default_prj)
            self.fields['demog_definition'].queryset = DemographicEntry.objects.filter(project=self.default_prj)
