...
def dm_folder(instance, filename):
    fldrTitle = ''.join([c for c in instance.title if c.isalnum() and ord(c) <= 127])
...
class Modeler(models.Model):
    ...
    email = models.EmailField(_("Email"), max_length=110, help_text=_('Enter the author email address as it should appear in DM metadata.'))
    project = models.ForeignKey(Project, verbose_name=_("Default Project"), to_field="prj_name", help_text=_('Choose the default Project.'), blank=True, null=True)
...
class Predicate(models.Model):
    ...
    """
    ns_abbrev = models.ForeignKey(NS, verbose_name=_("NS Abbreviation"), help_text=_('Select a valid namesspace abbreviation.'))
...
class PredObj(models.Model):
    ...
    """
    predicate = models.ForeignKey(Predicate, verbose_name=_("Predicate"), help_text=_("Select a predicate to define the RDF triple."), blank=True, null=True)
    object_uri = models.CharField(_("Object URI"), max_length=5000, help_text="Enter an IRI for the object of the RDF triple.", blank=True, default='')
    project = models.ForeignKey(Project, verbose_name=_("Project Name"), to_field="prj_name", help_text=_('Choose the name of the Project.'))

    def __str__(self):
        return self.project.prj_name + ' : ' + self.predicate.__str__() + " --> " + self.object_uri.strip()
...
class Common(models.Model):
    ...
    description = models.TextField(_('description'),help_text=_("Enter a free text description for this complexType. Include a usage statement and any possible misuses. This is used as the annotation for the MC."), null=True)
    pred_obj = models.ManyToManyField(PredObj, verbose_name=_("Predicate - Object"), help_text=_("Select or create a new set of Predicate Object combinations as semantic links."))
    ...
    creator = models.ForeignKey(Modeler, verbose_name="Creator", blank=True, related_name='%(class)s_related_creator', default=1)
    edited_by = models.ForeignKey(Modeler, verbose_name="Last Edited By", blank=True, related_name='%(class)s_related_edited_by', default=1)
    r_code = models.TextField(_("R Code"), help_text="This is only writable from the DMGEN, not via user input. It contains the code required for each component to create a function for the R data analysis.", blank=True, null=True, default='')
...
class DvLink(DvAny):
    ...
    """
    relation = models.CharField(_('relation'),max_length=110, help_text=_("The relationship describing the link. Usually constrained by an ontology such as <a href='http://www.obofoundry.org/cgi-bin/detail.cgi?id=ro'>OBO RO</a>."))
    relation_uri = models.CharField(_('relation URI'),max_length=255, help_text=_("The relationship URI. Points to the vocabulary, ontology, etc that provides the relation."))
...
class DvFile(DvAny):
    ...
    """
    MODE_TYPES = (('select','Select Mode:'),('url','Link via a URL'),('text','Embed a text file'),('binary','Embed a binary file'))
    ...
    content_mode = models.CharField(_("Content Mode"), default='Select Mode:', help_text=_("Select how the content will referenced, either via a URL or included in the data instance. Example text is; XML, JSON, SQL, etc. Example binary is; MP#, MP4, PNG, etc."), choices=MODE_TYPES, max_length=6)
...
class DvOrdinal(DvOrdered):
    ...
    annotations = models.TextField(_('Symbols Definitions'),blank=True, help_text=_("Enter a URI for a definitions for each symbol. One per line."))
...
class DvQuantified(DvOrdered):
    ...
    min_inclusive = models.IntegerField(_('minimum inclusive'),help_text=_("Enter the minimum (inclusive) value for this concept."), null=True, blank=True)
    max_inclusive = models.IntegerField(_('maximum inclusive'),help_text=_("Enter the maximum (inclusive) value for this concept."), null=True, blank=True)
    min_exclusive = models.IntegerField(_('minimum exclusive'),help_text=_("Enter the minimum (exclusive) value for this concept."), null=True, blank=True)
    max_exclusive = models.IntegerField(_('maximum exclusive'),help_text=_("Enter the maximum (exclusive) value for this concept."), null=True, blank=True)
...
class DvCount(DvQuantified):
    ...
    standardized units)
    """
    units = models.ForeignKey(Units, verbose_name=_('units'),related_name='%(class)s_related_units', null=True, help_text=_("Choose a units of measurement of this concept."))
...
class DvRatio(DvQuantified):
    ...
    num_units = models.ForeignKey(Units, verbose_name=_('units'),related_name='%(class)s_related_num_units', null=True, blank=True,help_text=_("Choose a units of measurement of this concept."))
    den_units = models.ForeignKey(Units, verbose_name=_('units'),related_name='%(class)s_related_den_units', null=True, blank=True,help_text=_("Choose a units of measurement of this concept."))
    ratio_units = models.ForeignKey(Units, verbose_name=_('units'),related_name='%(class)s_related_ratio_units', null=True, blank=True,help_text=_("Choose a units of measurement of this concept."))
...
class DvTemporal(DvOrdered):
    ...
    allow_datetimestamp = models.BooleanField(_('allow datetimestamp'),default=False, help_text=_('Check this box if a dateTimeStamp is allowed.'))
...
class Entry(Common):
    ...
    """
    language = models.CharField(_("language"), max_length=40, choices=LANGUAGES, default='en-US', help_text=_('Choose the language of this Entry.'))
    ...
    data = models.ForeignKey(Cluster, verbose_name=_('entry data'), related_name='%(class)s_related', help_text=_("You must select the Cluster that is the structure for this Entry." ))
...
class DM(models.Model):
    ...
    subject = models.CharField(_('subject'),max_length=255, help_text=_("Enter a semi-colon separated list of keywords. Usually MeSH terms"))
    ...
    dc_type = models.CharField(_('type'),max_length=110, editable=False, default="MLHIM Data Model (DM)")
    identifier = models.CharField(_('identifier'),max_length=110, editable=False, default="ccd-") # concat the uuid in the template
    description = models.TextField(_('description'),help_text=_("Enter a general description of the purpose of this DM."))
    ...
    dc_language = models.CharField(_("language"),max_length=6,default="en-US",choices=LANGUAGES, help_text=_('The written language of the DM.'))
    entry = models.ForeignKey(Entry, verbose_name=_("entry"), help_text=_('Select the correct Entry for this DM.'))
    ...
    pred_obj = models.ManyToManyField(PredObj, verbose_name=_("Predicate - Object"), help_text=_("Select or create a new set of Predicate Object combinations as semantic links."))
    schema_code = models.TextField(_("Schema Code"),  help_text="This is only writable from the DMGEN, not via user input. It contains the code required for each component to create an entry in a DM.", blank=True, null=True, editable=True)
