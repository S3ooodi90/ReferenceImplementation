...
The MLHIM2 Reference Model code to write DM schemas.
Copyright 2014-2015, Timothy W. Cook, All Rights Reserved.

This code is accessed from DM model class in model.py
...
class DMPkg(object):
    ...
    def xsdMetadata(self):
        ...
        mds += '    <dc:subject>'+escape(self.dm.subject.strip())+'</dc:subject>\n'
    ...
    def processDM(self):
        ...
        if msg[1] == messages.SUCCESS:
            self.xsd += '<xs:element name="'+self.dm.identifier+'" type="s3m:mc-'+self.dm.ct_id+'"/>\n'
            ...
            if len(self.dm.pred_obj.all()) != 0:
                for po in self.dm.pred_obj.all():
                    self.xsd += "        <"+po.predicate.ns_abbrev.__str__()+":"+po.predicate.class_name.strip()+" rdf:resource='"+ quote(po.object_uri) +"'/>\n"
    ...
    def processEntry(self,entry):
        ...
        if not entry.published:
            ...
            if entry.data:
                ...
                msg = ("The Entry is missing a Data (ClusterType) model.", messages.ERROR)
                return(msg)
        ...
        if party.details:
            if self.registerUUID(party.details.ct_id, 'ClusterType'):
                if len(party.details.schema_code) < 10: # len 10 was arbitrarily chosen as an obviously incorrect code length.
                    msg = ("Something happened to your MC code. Check that Cluster: "+party.details.label+" is published.", messages.ERROR)
                    return(msg)
    ...
    def processParticipation(self,part):
        ...
        if part.performer:
            if self.registerUUID(part.performer.ct_id, 'PartyType'):
                if len(part.performer.schema_code) < 10: # len 10 was arbitrarily chosen as an obviously incorrect code length.
                    msg = ("Something happened to your MC code. Check that Party: "+part.performer.label+" is published.", messages.ERROR)
        ...
        if aud.system_user:
            if self.registerUUID(aud.system_user.ct_id, 'PartyType'):
                if len(aud.system_user.schema_code) < 10: # len 10 was arbitrarily chosen as an obviously incorrect code length.
                    msg = ("Something happened to your MC code. Check that Party: "+aud.system_user.label+" is published.")
                    return(msg)
        ...
        if aud.location:
            if self.registerUUID(aud.location.ct_id, 'ClusterType'):
                if len(aud.location.schema_code) < 10: # len 10 was arbitrarily chosen as an obviously incorrect code length.
                    msg = ("Something happened to your MC code. Check that Cluster: "+aud.location.label+" is published.", messages.ERROR)
                    return(msg)
    ...
    def processAttestation(self,att):
        ...
        if att.committer:
            if self.registerUUID(att.committer.ct_id, 'PartyType'):
                if len(att.committer.schema_code) < 10: # len 10 was arbitrarily chosen as an obviously incorrect code length.
                    msg = ("Something happened to your MC code. Check that Party: "+att.committer.label+" is published.")
                    return(msg)
    ...
    def getXSD(self):
        ...
        # create the substitution groups needed and close the schema and instance
        if msg[1] == messages.SUCCESS:
...
def generateDM(dm, request):
    ...
    # Cleanup - Remove old files if this DM has been generated before. Set Published as false.
    dm.html_file.delete()
    dm.xml_file.delete()
    dm.json_file.delete()
    dm.xsd_file.delete()
    ...
    # set the umask so we can create files in the directory we create via Apache/NGINX.
    ...
    if msg[1] == messages.SUCCESS:
        ...
        dm.xsd_file.close()
        f.close()
        lf = os.open(dm_dir+'/dm-'+ dm.ct_id+'.xsd', os.O_RDWR|os.O_CREAT )
        ...
        dm.html_file.close()
        f.close()
        lf = os.open(dm_dir+'/dm-'+ dm.ct_id+'.html', os.O_RDWR|os.O_CREAT )
        ...
        dm.xml_file.close()
        f.close()
        lf = os.open(dm_dir+'/dm-'+ dm.ct_id+'.xml', os.O_RDWR|os.O_CREAT )
        ...
        sha1.close()
        f.close()
        lf = os.open(dm_dir+'/dm-'+ dm.ct_id+'.sha1', os.O_RDWR|os.O_CREAT )
        ...
        xmlfile = open(dm_dir+'/dm-'+dm.ct_id+'.xml', 'r', encoding="utf-8")
        xml = xmlfile.read()
        xmlfile.close
        ...
        messages.add_message(request, messages.SUCCESS, "Wrote the R Studio project files.")
        ...
        for dirname, subdirs, files in os.walk(dm_dir):
            for filename in files:
                zf.write(os.path.join(dirname, filename), dirname.replace('../dmlib/','')+'/'+filename)

        zf.close()
        messages.add_message(request, messages.SUCCESS, "Created a ZIP of all the files.")
...
def gen_metadataR(self):
    ...
    rstr += "dmuri <- function(){\n"
    rstr += "    return('http://dmgen.s3model.com/dmlib/dm-"+self.ct_id+".xsd')\n"
    rstr += "}\n"
    rstr += "\n"
